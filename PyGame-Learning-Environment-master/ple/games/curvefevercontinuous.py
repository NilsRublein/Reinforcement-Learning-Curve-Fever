...
class Player:
    ...
    def update(self,dt):
        ...
        #print("array self.angle: ",self.angle)
        
        if not self.angle: # fixing empty array
            self.angle = 0
...
class CurveFeverContinuous(base.PyGameWrapper):
        ...
        def __init__(self, width=480, height=480):
               ...
               # still discrete, ple wants me to have a dict ... actions = {"Cont_action": spaces.Box(low=-10,high=10,shape=(1,))}
               # Also need to include noop as 0
               # Although, 0 is already include in [-10,10]
                #actions =spaces.Box( np.array([-10,0]), np.array([+10,0]) )
                actions =spaces.Box( np.array([-10]), np.array([+10] ) )
                ...
                base.PyGameWrapper.__init__(self, width, height, actions=actions, discrete = False)
        ...
        def step(self, dt):
                ...
                #print("Curve Action: ", self.action)
                #print("Curve Angle: ", self.player.angle)
                
                if not self.action: # if action is empty
                    self.player.angle += 0
                else:
                    self.player.angle += self.action[0]
        ...
        def reset(self):
            self.observation_space = spaces.Box(low=0, high=WINWIDTH, shape=(12,), dtype=np.uint8)
            self.last_action = [] # change to 0? or rand(-10,10)?
            self.action = [] # change to 0? or rand(-10,10)?
...
if __name__ == "__main__":
    ...
    pygame.init()
    game = CurveFeverContinuous(width=WINWIDTH, height=WINHEIGHT)
